...
describe('disableDownload', () => {
  ...
  it('always return true when the system is windows and the arch is not arm64', () => {
    overrideRendererPlatform('win32');
    overrideRendererArch('x64');

    expect(disableDownload('6.0.0')).toBe(true);
    expect(disableDownload('6.0.8')).toBe(true);
    expect(disableDownload('7.0.0')).toBe(true);
    expect(disableDownload('8.0.0')).toBe(true);
  });

  it('always return true when the system is windows and the arch is arm64', () => {
    overrideRendererPlatform('win32');
  ...
  it('always return false when the system is linux', () => {
    ...
    expect(disableDownload('11.0.0')).toBe(true);
    expect(disableDownload('12.0.0')).toBe(true);
  });

  it('always return true when the system is macOS and the arch is not arm64', () => {
  ...
  it('always return false when the system is macOS and the arch is arm64', () => {
    overrideRendererPlatform('darwin');
    overrideRendererArch('arm64');
